...
import com.rpcclient.rpc.interceptor.RpcInterceptorChain;
import com.rpcclient.rpc.interceptor.RpcInterceptorTerminal;
...
public class RpcCaller implements InvocationHandler {
    private final RpcContext context;
    private final List<RpcInterceptor> interceptors;

    public RpcCaller(RpcContext context, List<RpcInterceptor> interceptors) {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        context.methodName = rpc.name();
        context.message =  args == null ? null : (com.google.protobuf.GeneratedMessageV3) args[0];

        RpcInterceptorTerminal terminal = new RpcInterceptorTerminal() {
            @Override
            public Transport.SimpleResponse invoke() {
                return context.transport.call(context.serviceName, context.methodName, context.message);
            }
        };

        RpcInterceptorChain chain = new RpcInterceptorChain(interceptors, 0, terminal);
        Transport.SimpleResponse response = (Transport.SimpleResponse) chain.process(context);
